...
public partial class ProfileSettingsView : UserControl
{
    ...
    private void OnGlobalUndoRedoKeyDown(object? sender, KeyEventArgs e)
    {
        ...
        if (HostTopLevel?.FocusManager?.GetFocusedElement() is TextBox)
            return;
        switch (e.Key)
        {
    ...
    public async Task<bool> OpenAsync(Uri? uri = null)
    {
        if (!_isOpen)
        {
    ...
    private void ButtonOpenClassPlanDetailsWindow_OnClick(object? sender, RoutedEventArgs e)
    {
        ...
        {
            _ = details.ShowDialog(window);
        }
        else
        {
            this.ShowErrorToast("课程表详情窗口尚未接入当前平台宿主。");
        }
    }
