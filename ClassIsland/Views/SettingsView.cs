...
/// 桌面端与移动端共用的设置视图交互逻辑。
/// </summary>
public sealed class SettingsView : SettingsShell, INavigationPageFactory
{
    ...
    private Frame NavigationFrame => NavigationFrameControl;

    public SettingsNewViewModel ViewModel { get; } = new();

    public PlatformCapabilities PlatformCapabilities => App.CurrentPlatformCapabilities;
    ...
    private TopLevel? HostTopLevel => TopLevel.GetTopLevel(this);

    private Window? HostWindow => HostTopLevel as Window;
    ...
    public SettingsView(IManagementService managementService, IHangService hangService,
        ILogger<SettingsView> logger, DiagnosticService diagnosticService, SettingsService settingsService,
        ...
        ITutorialService tutorialService)
    {
        ...
        NavigationItemInvoked += NavigationView_OnItemInvoked;
        NavigationBackRequested += NavigationView_OnBackRequested;
        NavigationLoaded += Control_OnLoaded;
        FrameNavigated += NavigationServiceOnLoadCompleted;
        FrameNavigating += NavigationServiceOnNavigating;
        BackClicked += BackButton_OnClick;
        TogglePaneClicked += TogglePaneButton_OnClick;
        RestartClicked += ButtonRestartApp_OnClick;
        OpenDrawerCommandExecuted += CommandBindingOpenDrawer_OnExecuted;
        CloseDrawerCommandExecuted += CommandBindingCloseDrawer_OnExecuted;
        RestartCommandExecuted += CommandBindingRestartApp_OnExecuted;
        ExperimentalSettingsClicked += MenuItemExperimentalSettings_OnClick;
        DebugWindowRuleClicked += MenuItemDebugWindowRule_OnClick;
        AddDesktopShortcutClicked += MenuItemAddDesktopShortcut_OnClick;
        AddStartMenuShortcutClicked += MenuItemAddStartMenuShortcut_OnClick;
        AddClassSwapShortcutClicked += MenuItemAddClassSwapShortcut_OnClick;
        DataTransferClicked += MenuItemDataTransfer_OnClick;
        OpenManagementSettingsClicked += MenuItemOpenManagementSettingsPage_OnClick;
        JoinManagementClicked += MenuItemJoinManagement_OnClick;
        ExitManagementClicked += MenuItemExitManagement_OnClick;
        AppLogsClicked += MenuItemAppLogs_OnClick;
        ExportDiagnosticInfoClicked += MenuItemExportDiagnosticInfo_OnClick;
        RestartToRecoveryClicked += MenuItemRestartToRecovery_OnClick;
        OpenLogFolderClicked += MenuItemOpenLogFolder_OnClick;
        OpenDataFolderClicked += MenuItemOpenDataFolder_OnClick;
        OpenAppFolderClicked += MenuItemOpenAppFolder_OnClick;
        SizeChanged += SettingsView_OnSizeChanged;
            ...
            foreach (var i in info)
            {
                ...
                if (i.GroupId != null && SettingsWindowRegistryService.Groups.TryGetValue(i.GroupId, out var group))
                {
                    ...
                    item = new NavigationViewItem()
                    {
                        ...
                        Content = group.Name,
                        Tag = i,
                        IsExpanded = App.IsPortableModeRequested && i.GroupId == "classisland.mobile"
                    };
    ...
    private void SettingsView_OnSizeChanged(object? sender, SizeChangedEventArgs e)
    {
    ...
    public async Task<bool> AuthorizeOpenAsync()
    {
        if (!await ManagementService.AuthorizeByLevel(
                ManagementService.CredentialConfig.EditSettingsAuthorizeLevel))
        {
            return false;
        }
    ...
    public async Task NavigateAsync(string key, Uri? uri = null)
    {
    ...
    public async Task NavigateUriAsync(Uri uri)
    {
        if (uri.Segments.Length > 2)
        {
            var uriSegment = uri.Segments[2].EndsWith('/') ? uri.Segments[2][..^1] : uri.Segments[2];
            await NavigateAsync(uriSegment, uri);
        }
    ...
    {
        ViewModel.DrawerContent = key == "ExperimentalSettings"
            ? ExperimentalSettingsContent
            : this.FindResource(key);
        ViewModel.IsDrawerOpen = true;
    }
    ...
    private async void MenuItemExitManagement_OnClick(object sender, RoutedEventArgs e)
    {
        ...
        catch (Exception ex)
        {
            ...
            {
                _ = CommonTaskDialogs.ShowDialog("无法退出管理", $"无法退出管理：{ex.Message}", window);
            }
            else
            {
                this.ShowErrorToast($"无法退出管理：{ex.Message}");
            }
        }
    }

    private async void MenuItemAppLogs_OnClick(object sender, RoutedEventArgs e)
    {
        if (App.IsPortableModeRequested &&
            await App.NavigatePortableAsync("logs"))
        {
            return;
        }
    ...
    private async void MenuItemExportDiagnosticInfo_OnClick(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            ...
            var topLevel = HostTopLevel ?? throw new InvalidOperationException("找不到设置视图的顶层宿主。");
    ...
    private async void MenuItemAddClassSwapShortcut_OnClick(object sender, RoutedEventArgs e)
    {
        ...
        if (topLevel is null)
        {
            this.ShowErrorToast("找不到设置视图的顶层宿主。");
            PopupHelper.RestoreAllPopups();
            return;
        }
        var file = await PlatformServices.FilePickerService.SaveFilePickerAsync(new FilePickerSaveOptions()
    ...
    {
        if (App.IsPortableModeRequested &&
            await App.NavigatePortableAsync("data-transfer"))
        {
            return;
        }
        ...
        {
            this.ShowErrorToast("数据迁移窗口尚未接入当前平台宿主。");
            return;
        }

        IAppHost.GetService<DataTransferWindow>().ShowDialog(window);
    }
    ...
    private void MenuItemJoinManagement_OnClick(object? sender, RoutedEventArgs e)
    {
        if (HostWindow is not { } window)
        {
            this.ShowErrorToast("加入集控窗口尚未接入当前平台宿主。");
            return;
        }
